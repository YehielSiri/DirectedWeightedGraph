...
public class window extends JFrame implements ActionListener {
	...
	public window() {
		super();
		init();
		this.panel = new PaintPanel(new Directed_Weighted_Graph());
		this.add(this.panel);
		this.setVisible(true);
	}
	...
	public JPanel select() {
		...
		for (int id : ((Directed_Weighted_Graph) panel.dg.getGraph()).getNodes().keySet()) nodes1.addItem(id);
		p.add(nodes1);
		JLabel helpButton = miniLabel("Select dest Node:");
		p.add(helpButton);
		JComboBox<Integer> nodes2 = new JComboBox<>();
		for (int id : ((Directed_Weighted_Graph) panel.dg.getGraph()).getNodes().keySet()) nodes2.addItem(id);
		p.add(nodes2);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == TspITEM) {
			...
			find.addActionListener(e14 -> {
				...
				for (int i = 0; i < nodes2.getItemCount(); i++)
					send.add(((Directed_Weighted_Graph) panel.dg.getGraph()).getNodes().get(nodes2.getItemAt(i)));
		...
		if (e.getSource() == addNode) {
			...
			JPanel p = new JPanel();
			int x = 0;
			while (((Directed_Weighted_Graph) panel.dg.getGraph()).getNodes().containsKey(x)) x++;
			...
			add.addActionListener(e13 -> {
				...
				d.addNode(new Node_Data(finalX, new Geo_Location("" + wr1.getText() + "," + wr2.getText() + ",0.0")));
				contain.removeAll();
		...
		if (e.getSource() == removeEdge) {
			...
			nodes1.addActionListener(e17 -> {
				...
				for (int id : ((Node_Data) ((Directed_Weighted_Graph) panel.dg.getGraph()).getNodes().get(x)).getEdges().keySet())
					nodes2.addItem(id);
				contain.validate();
				contain.repaint();
				setVisible(true);
